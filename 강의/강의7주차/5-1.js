...
// 만약 캐시의 사이즈가 5인 작업이 [2, 3, 1, 6, 7]순으로 저장되어 있다면
...
function solution(S, N, num) {
    ...
    num.forEach((number) => {
        ...
        if(!cache.includes(number)) {
            cache[S - 1] = number
            for(let i = S-1; i >= 1; i--) {
                cache[i] = cache[i - 1] // 뒤로 한 칸씩 밀려 복사하기
            } cache[0] = number // 최근 작업을 맨 앞에 할당
        }
        // 2. cache hit
        else if(cache.includes(number)) {
            for(let i = cache.findIndex(a => a === number); i >= 1; i-- ) {
                cache[i] = cache[i - 1]
            } cache[0] = number
        }
    })
